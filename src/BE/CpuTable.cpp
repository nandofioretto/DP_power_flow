...
CpuTable::CpuTable(Constraint::ptr con)
{
    ...
    // New Ordered Constraint Scope (acending order) - variable to eliminate appear as last.
    ...
    for (int i = 0; i < arity; i++)
    {
        ...
        domMax[i] = con->getVariableAt(newScope[i])->getDomSize();
    ...
    std::vector<std::vector<value_t>> reorderedDomainsInScopeOfConstraint;
    for (int i=0; i<scope.size(); i++)
        reorderedDomainsInScopeOfConstraint.push_back(scope[i]->getValues());

    // Gen Permutations
    combinatorics::Permutation<value_t> perm(reorderedDomainsInScopeOfConstraint);
...
CpuTable::CpuTable(std::vector<Agent::ptr> agts)
{
    ...
    for (int i = 0; i < agts.size(); i++)
    {
        ...
        // Get bounds (in the new order)
        domMax[i] = CpuTable::scope[i]->getDomSize();
        tableSize *= domMax[i];
    }
    tableSize *= agts.size();
    Cpu::Info::incrGlobalMemory(tableSize*sizeof(value_t));
